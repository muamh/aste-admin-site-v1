...
const UserCard = ({ user }) => {
    ...
    const eliminazione = () =>{
        const result = api.post("/api/v1/users/single/user/deleting/protected/checkpoint", {uid: user.uid, identificator: sessionStorage.getItem("identificator")}).then(result => result.data);
        alert(result);
        ...
        return (
            <tr height='50'>
                ...
                <td>{user.nome}</td>
                <td>
                    <Link to={`/single/user/details/${user.uid}`}>
                ...
                <td>
                    <Link to={`/single/user/modifications/${user.uid}`}>
                ...
                <div className="row g-4" >
                    <div class="wrap">
                        <div style={{ marginLeft: '1%' }}>
                            <h1 class="display-1">Elenco utenti</h1>
                        </div>
                    </div>
